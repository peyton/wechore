...
if [ -z "$key_source" ]; then
  ...
  hint "  APP_STORE_CONNECT_API_KEY_P8=<raw PEM>                        (CI preferred)"
  hint "  APP_STORE_CONNECT_API_KEY_P8_BASE64=<base64>                  (CI fallback)"
  hint ""
  hint "To create the fallback base64 value for CI:"
  hint "  base64 -i AuthKey_<KEY_ID>.p8 | tr -d '\\n'"
  hint ""
  hint "GitHub secret: APP_STORE_CONNECT_API_KEY_P8 in 'testflight' environment"
else
...
printf '    APP_STORE_CONNECT_API_KEY_P8         %b# raw AuthKey_<ID>.p8 contents%b\n' "$DIM" "$RESET"
printf '    APP_STORE_CONNECT_API_KEY_P8_BASE64  %b# fallback base64 form, optional%b\n' "$DIM" "$RESET"
printf '\n'
printf '  %bVariables:%b\n' "$BOLD" "$RESET"
